...
- ４つのサービスのtraefik.frontend.ruleのドメイン名を自分のものに変更する。
...
上記で設定したdocker-compose.ymlを実行するために，VMインスタンスにSSH接続します。ウィンドウの右上の設定っぽいボタンをクリックするとアップロードの選択肢がでてくると思うので，それを使って，さきほどの"docker-compose.yml"をアップロードします。それができたら，以下を打ち込んで，docker-compose.ymlを実行します。ダウンロードとかあるので時間がかかることがあります。しばらくして，各サービースのコンテナがdoneって表示されたら，成功です。
