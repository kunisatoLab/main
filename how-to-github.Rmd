...
**アカウントの用意ができたら，slackで国里にアカウント名を知らせてください。**　国里が学生用プライベートリポジトリとゼミ生用マテリアルリポジトリを共有します。
...
RStudioで「Tools」をクリックして，「Global Options...」をクリックします。「Git/SVN」で「Create RSA Key...」をクリックします。

![](fig/git_ssh1.png)

Passphraseを設定したほうがGitHubへのアクセスに対しては強固ではありますが，Passphraseが有用な状況というのは，サーバーに侵入されている状態なので，あまり意味がないので，ここでは設定をしません。そのままCreateをクリックします。

![](fig/git_ssh2.png) 

以下のようにRSA Keyが作られます。とりあえず，「close」を押して閉じます。「View Public Key」をクリックして，鍵を表示させて，コピーします。
...
適当にタイトルを決めて(例えば，「lab rstudio server」とか)，上記のRSA Keyをコピペします。これで登録完了です。
