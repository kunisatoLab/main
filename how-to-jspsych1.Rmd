...
さて，上記の```set_jsPsych(file_name = "hello")```で作成されたindex.Rmdを開いてください。以下のように，すでに書き込まれているかと思います。まずは，特に考えずに，赤い四角で囲ったKnitをクリックしてみましょう。Knitをクリックすると，RmdからHTMLファイルが作成されて，画面上に「こんにちは！」というのが出てきます。それができていたら，成功です。
...
jsPsychの本体のコードは，チャンクを{js}とします（このチャンク内でjsPsychを使ったjsコードを書きます）。こちらがjsPsychで課題を作成する際の本体になります。jsPsychRmdで```set_jsPsych(file_name = "hello")```した場合は，以下の内容が事前に入っています。
