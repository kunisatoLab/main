...
まずは，これまでと同様なαもβも存在する強化学習モデルでデータを生成します。それに対して，それと一致したモデル（上記のq_learning_prior_WBIC.stan）と一致しないモデル（以下のq_learning_prior_non_beta_WBIC.stan）を使って推定することで，モデルリカバリーをしてみます。
...
for (i in 1:n_simulation) {
  ...
  tryCatch({
    ...
    better_with_beta <- better_with_beta + ifelse(WBIC_with_beta < WBIC_no_beta, 1, 0)
  },error = function(e) {message(e)})
}

fit_both_sim_both <- better_with_beta/n_simulation
fit_alpha_sim_both <- (n_simulation-better_with_beta)/n_simulation
```
...
fit_both_sim_alpha <- better_with_beta/n_simulation
fit_alpha_sim_alpha <- (n_simulation-better_with_beta)/n_simulation
```
...
```{r}
simulated_model <- factor(c("alpha & beta","alpha & beta","alpha","alpha"), levels = c("alpha & beta","alpha"))
fit_model <- factor(c("alpha & beta","alpha","alpha & beta","alpha"), levels = c("alpha","alpha & beta"))
Probability <- c(fit_both_sim_both, fit_alpha_sim_both, fit_both_sim_alpha, fit_alpha_sim_alpha)
