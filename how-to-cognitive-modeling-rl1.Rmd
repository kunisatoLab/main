...
以降では，強化学習モデルを用いた認知モデリングをしてみましょう。厳密な理解よりは，手を動かしながら，学ぶことを重視して進めていきます。
...
- Cools et al.(2001)の逆転学習課題をベースにしている：合計80試行（前半40試行，後半逆転して40試行），フィードバックは顔（ポジティブが緑の嬉しい顔，ネガティブが赤の悲しい顔)，報酬:罰比は，80:20。選択刺激の呈示位置は左右，刺激の選択画面は2000ms呈示されて（2000msを超えると遅いことを伝える「2秒以内に選択してください」），フィードバックは２つの刺激の間に500ms呈示。
...
s1は，前半40試行において20％の確率で報酬のフィードバックがなされ，後半40試行で80%の確率でフィードバックがなされます（s2はその逆になる）。prob_s1とprob_s2に確率を設定して，reward_s1とreward_s2でランダムに80%や20%の確率で報酬が出てくるのを設定します（これはifelse関数を使って，0から1の範囲で一様分布から発生させた乱数が設定した確率(prob_s1やprob_s2)より小さければ1=reward,大きければ0=punishmentを生成します）。
...
- 強化学習(Sutton & Barto,”Reinforcement Leatning”,2000)とは，長期的にみて報酬を最大化するために，環境の状態に対して，特定の行動を学習するエージェントについての計算論的な枠組みのことを指す。

- 強化学習において，エージェントは，不確実な環境で，自分の行った行動と環境の状態の評価（報酬）から学習する（教師なし学習）。
方策(ある状態でどういう行動をどのくらいの確率で行うか)と価値関数（ある状態や行動の評価）をもって環境と相互作用する。

- 将来の報酬と罰を予測することの学習（古典的条件づけ）と将来の報酬を最大化する行動を選択することの学習（道具的条件づけ）をモデル化(Kurth-Nelson et al. 2017)。

- スロットマシーンが２つあり，どちらかのスロットの方が報酬が出るように設定されている

- 何度もスロットをする中で，段々とそのマシーンの価値が定まっていく（マシーンの価値を学習する）。
試行ごとの選択における価値の更新過程を，強化学習モデルでモデリングできる。


- Ｑlearning Model(Watkins, Machine Learning,1989)：TD誤差(δ)をできるだけ小さくするように，Q値の更新していくことで，最適な行動価値関数を学習するモデル。

α：学習率（大きいほど学習が早い）
γ：割引率（大きいほど先を見通した行動が出来る）

- 一般的な，Q learningモデルにおける行動の価値を表現するQ値の更新式は以下のようになります。

- 強化学習はマルコフ決定過程に基づく（離散化した状態において，行動をとると，確率的に次の状態に遷移する。状態遷移の確率は，P(st+1|st,at)になる）
報酬(Rt)は，数ステップ後も考慮して計算される。
報酬を加算しつづけると無限になるので，割引率(discount rate)のγをつける(0≦γ＜1)。
γkは，ステップ(k)が大きくなるほど小さくなる（報酬を割り引く）。
...
- 今回の逆転学習課題の場合は，状態が1つの特殊なQ learningになります。γのついた項は不要になり，報酬も選択した時点の報酬で更新します。ある状態sにおけるある行動aの価値Qを以下の式で更新します。
...
グリーディ法
最もQ値の高い行動を必ず選択する
現状で得た知識を最大限利用(Exploitation)する方略。
選択したことがない，より良い選択肢に気づけない。

ランダム法
あらゆる行動を等確率で選択する。
現状で得た知識よりも良い選択肢がないか探索(Exploration)する方略。
ランダムなので，グリーディほど報酬を獲得できない。

探索（ランダム法）と利用（グリーディ法）のバランスをとることが重要


ソフトマックス行動選択では，逆温度(inverse temperature: β)で探索と利用のバランスをとっている。βは0以上の値をとり，β=0でランダム法，βが大きくなるとグリーディ法に近づく。
...
Q learning Modelでは，ある選択肢を選ぶ価値は，TD誤差に学習率(α)を掛けたものによって更新される。
期待と報酬にズレがあると学習が進む！
学習率(α)が大きいと学習が早く進む！
