...
```
q_non_info_single <- cmdstan_model('q_learning_non_info_single.stan')
```
...
ベイズ推定でもパラメータリカバリをしてみましょう。ただ，これは結構長くなると思います。
...
parameter_recovery_non_info <- data.frame(alpha_true = alpha_true,
                                     beta_true = beta_true,
...
パラメータリカバリーのチェックをしましょう。散布図を書いて，真値（研究者がデータ生成時に設定した値）と推定された値が強い相関を示しているか確認します（データ生成時や推定時に確率的な変動が生じるので，完全一致はありません）。最尤推定同様に，βがすごく大きくなることがあるのは，気になるところです。

```{r}
parameter_recovery_non_info %>% 
  ggplot(aes(x = alpha_true, y = alpha_estimated)) +
  geom_point()
parameter_recovery_non_info %>%
  ggplot(aes(x = beta_true, y = beta_estimated)) +
...
```
q_weak_info_single <- cmdstan_model('q_learning_weak_info_single.stan')
```

まず，先程のdata_1で試してみます。

```{r include=FALSE}
q_weak_info_single_cmdstan <- q_weak_info_single$sample(
  data = list(trial = nrow(data_1),
              reward = data_1$reward,
              choice = data_1$choice + 1),
  seed = 123,
...
```{r}
q_weak_info_single_cmdstan$summary()
```

以下はMCMCについての診断結果です。収束診断の問題もないかと思います。

```{r}
q_weak_info_single_cmdstan$cmdstan_diagnose()
```
...
parameter_recovery_week_info <- data.frame(alpha_true = alpha_true,
                                     beta_true = beta_true,
                                     alpha_estimated = alpha_estimated,
                                     beta_estimated = beta_estimated)
```
...
mcmc_non_info <- q_non_info_single$sample(
...
mcmc_non_info $summary()
mcmc_non_info $cmdstan_diagnose()
```
...
```{r}
mcmc_non_info <- as_draws_df(mcmc_cmdstan$draws())
# alphaのtrace plot
mcmc_non_info %>%
  mutate(chain = as.factor(.chain)) %>% 
  ggplot(aes(x = .iteration, y = alpha, group = .chain, color = chain)) +
...
mcmc_weak_info <- q_weak_info_single$sample(
        data = list(trial = nrow(data_individual),
                   reward = data_individual$reward,
                   choice = data_individual$choice + 1),
        seed = 123,
...
mcmc_weak_info$summary()
mcmc_weak_info$cmdstan_diagnose()
```
...
```{r}
mcmc_weak_info <- as_draws_df(mcmc_cmdstan$draws())
# alphaのtrace plot
mcmc_weak_info %>%
  mutate(chain = as.factor(.chain)) %>% 
  ggplot(aes(x = .iteration, y = alpha, group = .chain, color = chain)) +
