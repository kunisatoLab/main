...
[強化学習モデル: 最尤推定](https://kunisatolab.github.io/main/how-to-cognitive-modeling-rl1.html)ではABCDとすすめてきました。今回は，ベイズ推定をcmdstanを使って行ってみます。そこで，一度Bに戻ってからDEFと進みます。

# 使用するRパッケージ

適宜必要なパッケージをインストールしてください。cmdstnrのインストールは，[こちら](https://mc-stan.org/cmdstanr/)をご確認ください。
...
＜前に国里が書いた文章を改変する＞
また，計算論的アプローチでは複数の生成モデルを用いて検討することが推奨される。特定の生成モデルAから生成されたシミュレーションデータに対して，複数の生成モデルA ,B,Cを用いてパラメータ推定をした場合に，AがBやCよりもモデル比較において良いモデルとされるのか確認する(Wilson & Collins, 2019)。これをモデルリカバリーと呼ぶ。モデルの比較には，BICなどの情報量規準やベイズファクターなどが使える。モデルリカバリーは，全ての生成モデルからシミュレーションデータを生成し，全ての生成モデルを用いてパラメータ推定を行うことを全ての組み合わせで行う。その結果は，特定のモデルの下でのシミュレーションデータから特定の生成モデルがモデル比較で選ばれる確率を全ての組み合わせで行列化した混合行列(Confusion Matrix)でまとめられる(Wilson & Collins, 2019)。パラメータリカバリーとモデルリカバリーを通して，改めて検討しているモデルが研究目的から妥当か，追加すべきモデルはないか，パラメータ推定において実験課題の条件や試行数は妥当かなどを検討し，適宜，(1)の実験課題と生成モデルの設定を再検討する。
